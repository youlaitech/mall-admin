...
import { Money, Refresh, RefreshLeft, Right, CircleCheckFilled, CircleCloseFilled } from '@element-plus/icons-vue';
...
function handleOrderPay() {
	// 数据校验
	if (
		(seataData.value.stockInfo.stockNum && seataData.value.stockInfo.stockNum != 999) ||
		...
		ElMessageBox.confirm('检查到当前数据已被污染，请先重置数据后尝试提交?', '警告', {
...
<template>
	<div class="app-container">
		<el-alert type="info">
			...
			<p style="font-size: 14px">
				<b> 博客教程：</b>
				<el-link type="primary" href="https://www.cnblogs.com/haoxianrui/" target="_blank"> https://www.cnblogs.com/haoxianrui</el-link>
			</p>
		</el-alert>
		...
		<el-card class="box-card" shadow="always" style="margin-top: 20px">
			<el-form :inline="true">
				<el-row>
					<el-col :span="20">
						<el-form-item>
							<el-switch v-model="submitData.openTx" active-value="" active-text="开启事务" inactive-text="关闭事务" />
						</el-form-item>
		...
		<el-row :gutter="10" style="margin-top: 20px" v-loading="loading">
						...
						<div style="margin-left: 10px">
							...
							<el-form-item label="库存数量:" style="display: flex">
								...
								<div v-if="cacheSeataData.stockNum" style="margin-left: 50px">
									<el-link v-if="cacheSeataData.stockNum != seataData.stockInfo.stockNum" type="success" :underline="false" :icon="CircleCheckFilled">
										修改成功
									</el-link>
									<el-link
										v-else-if="cacheSeataData.stockNum == seataData.stockInfo.stockNum"
										type="danger"
										:underline="false"
			...
			<el-col :span="8" :xs="24" class="card-panel__col">
				<el-card class="box-card" shadow="always">
					...
					<div style="display: flex">
						<el-image style="width: 100px; height: 100px" :src="seataData.accountInfo.avatarUrl" fit="fill" />
						<div style="margin-left: 10px">
							...
							<el-form-item label="会员余额:">
								...
								<div v-if="cacheSeataData.balance" style="margin-left: 50px">
									<el-link v-if="cacheSeataData.balance != seataData.accountInfo.balance" type="success" :underline="false" :icon="CircleCheckFilled">
										修改成功
									</el-link>
									<el-link
										v-else-if="cacheSeataData.balance == seataData.accountInfo.balance"
										type="danger"
										:underline="false"
										:icon="CircleCloseFilled"
									>
			...
			<el-col :span="8" :xs="24" class="card-panel__col">
				<el-card class="box-card" shadow="always">
					<template #header>
						...
						订单信息
						<el-checkbox v-model="submitData.orderEx" :label="true" style="float: right; color: #f56c6c"> 搞点异常</el-checkbox>
					</template>
					...
					<el-form-item label="订单状态:">
						...
						<div v-if="cacheSeataData.balance" style="margin-left: 50px">
							<el-link v-if="cacheSeataData.status != seataData.orderInfo.status" type="success" :underline="false" :icon="CircleCheckFilled">
								修改成功
							</el-link>
							<el-link v-else type="danger" :underline="false" :icon="CircleCloseFilled"> 修改失败 </el-link>
						</div>
					</el-form-item>
				</el-card>
			</el-col>
